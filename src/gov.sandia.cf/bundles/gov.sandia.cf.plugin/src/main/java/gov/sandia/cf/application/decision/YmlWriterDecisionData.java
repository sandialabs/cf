 ...
 *
 */
public class YmlWriterDecisionData implements IYmlDataWriter {

	/**
	 * the logger
	 */
	private static final Logger logger = LoggerFactory.getLogger(YmlWriterDecisionData.class);
	...
	public void writeData(final Yaml yamlEngine, final File cfDataFile, final Map<ExportOptions, Object> exportOptions,
		...
		if (!exportOptions.containsKey(ExportOptions.DECISION_INCLUDE)
				|| !Boolean.TRUE.equals(exportOptions.get(ExportOptions.DECISION_INCLUDE))) {
			logger.warn("Analyst Decision is not selected to be included. Nothing to write into file {}", //$NON-NLS-1$
		...
		try (Writer writer = new StringWriter()) {
			...
			yamlEngine.dump(ExportOptionsMapper.getYmlDecisionData(exportOptions), writer);
