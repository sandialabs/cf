...
public class PIRTApplication extends AApplication implements IPIRTApplication {
	...
	public List<QuantityOfInterest> getQoIByModelAndParent(Model model, QuantityOfInterest parent) {
		...
		logger.debug("PIRTApplication: getQoIByModelAndParent"); //$NON-NLS-1$

		// Filter
		Map<EntityFilter, Object> filters = new HashMap<>();
		filters.put(QuantityOfInterest.Filter.MODEL, model);
		filters.put(QuantityOfInterest.Filter.PARENT, (parent == null ? NullParameter.NULL : parent));

		// Query
		return this.getDaoManager().getRepository(IQuantityOfInterestRepository.class).findBy(filters);
	}
	...
	public void reorderAllQuantityOfInterest(Model model, User user) throws CredibilityException {
		...
		List<QuantityOfInterest> sameGroupQuantityOfInterestList = getRootQoI(model);

		if (sameGroupQuantityOfInterestList == null) {
			return;
		}

		// construct data
		sameGroupQuantityOfInterestList.sort(
				Comparator.comparing(QuantityOfInterest::getGeneratedId, new StringWithNumberAndNullableComparator()));

		// set id for parents
		int index = 1;
		for (QuantityOfInterest qoi : sameGroupQuantityOfInterestList) {
	...
	public void reorderQuantityOfInterestAtSameLevel(QuantityOfInterest toMove, User user) throws CredibilityException {
		...
		List<QuantityOfInterest> sameGroupQuantityOfInterestList = getQoIByModelAndParent(toMove.getModel(),
				toMove.getParent());
	...
	public void reorderQuantityOfInterest(QuantityOfInterest toMove, int newIndex, User user)
		...
		// reorder
		List<QuantityOfInterest> reorderedList = IDTools.reorderList(sameGroupQuantityOfInterestList, toMove, newIndex);

		// set id for parents
		int index = 1;
		final String groupIdLabel = toMove.getParent() == null ? RscTools.empty() : toMove.getParent().getGeneratedId();
		for (QuantityOfInterest qoi : reorderedList) {
			String elementId = null;
			if (toMove.getParent() == null) {
				elementId = IDTools.generateAlphabeticId(index - 1);
			} else if (toMove.getLevel() % 2 == 0) {
	 ...
	 */
	@Override
	public void reorderPhenomenonGroups(PhenomenonGroup toMove, int newIndex) throws CredibilityException {
		...
		List<PhenomenonGroup> phenomenonGroups = getPhenomenonGroups().stream()
				.filter(g -> toMove.getQoi().equals(g.getQoi())).collect(Collectors.toList());
		...
		// reorder
		List<PhenomenonGroup> reorderedList = IDTools.reorderList(phenomenonGroups, toMove, newIndex);
	...
	public void reorderPhenomenaForGroup(PhenomenonGroup toMove) throws CredibilityException {
		...
		// get phenomenon list
		List<Phenomenon> phenomenonList = toMove.getPhenomenonList();
		...
		// construct data
		phenomenonList.sort(Comparator.comparing(Phenomenon::getIdLabel, new StringWithNumberAndNullableComparator()));

		// set id for parents
		int index = 1;
		final String groupIdLabel = toMove.getIdLabel();
	 ...
	 */
	@Override
	public void reorderPhenomena(Phenomenon toMove, int newIndex) throws CredibilityException {

		logger.debug("Reorder phenomena"); //$NON-NLS-1$

		int startPosition = IDTools.reverseGenerateAlphabeticIdRecursive(IDTools.ALPHABET.get(0));

		if (toMove == null || toMove.getPhenomenonGroup() == null || newIndex < startPosition) {
			return;
		}

		List<Phenomenon> phenomenonList = getPhenomena().stream()
				.filter(p -> toMove.getPhenomenonGroup().equals(p.getPhenomenonGroup())).collect(Collectors.toList());
		...
		// reorder
		List<Phenomenon> reorderedList = IDTools.reorderList(phenomenonList, toMove, newIndex);

		// set id for parents
		int index = 1;
		final String groupIdLabel = toMove.getPhenomenonGroup() == null ? RscTools.empty()
				: toMove.getPhenomenonGroup().getIdLabel();
		...
		// refresh group
		refresh(toMove.getPhenomenonGroup());
	}
