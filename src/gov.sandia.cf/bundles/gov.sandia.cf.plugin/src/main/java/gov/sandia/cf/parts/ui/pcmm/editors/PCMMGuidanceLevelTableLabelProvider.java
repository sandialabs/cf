...
public class PCMMGuidanceLevelTableLabelProvider extends ATooltipTableLabelProvider implements IColorProvider {
	/**
	 * Tree
	 */
	...
	protected Font getFont(Object element, int columnIndex) {
		...
		FontData defaultFontData = (defaultFont.getFontData() != null && defaultFont.getFontData().length > 0)
				...
				: new FontData();
		if (columnIndex == NAME_COLUMN_INDEX && (element instanceof PCMMElement || element instanceof PCMMSubelement
				|| (!isTreeWithColumn && (element instanceof PCMMLevel || element instanceof PCMMLevelDescriptor)))) {
