...
/**
 * This class write credibility Uncertainty data. It uses snakeyml library to
 * create the export.
 * 
 * @author Didier Verstraete
 *
 */
public class YmlWriterSystemRequirementData implements IYmlDataWriter {

	/**
	 * the logger
	 */
	private static final Logger logger = LoggerFactory.getLogger(YmlWriterSystemRequirementData.class);
	...
	public void writeData(final Yaml yamlEngine, final File cfDataFile, final Map<ExportOptions, Object> exportOptions,
		...
		if (!exportOptions.containsKey(ExportOptions.SYSTEM_REQUIREMENT_INCLUDE)
				|| !Boolean.TRUE.equals(exportOptions.get(ExportOptions.SYSTEM_REQUIREMENT_INCLUDE))) {
			logger.warn("System Requirement is not selected to be included. Nothing to write into file {}", //$NON-NLS-1$
		...
		try (Writer writer = new StringWriter()) {
			...
			yamlEngine.dump(ExportOptionsMapper.getYmlRequirementData(exportOptions), writer);
