...
public class YmlReaderUncertaintySchema implements IYmlReader<UncertaintySpecification> {
	...
	public boolean isValid(File ymlSchema) {
		...
		try (FileReader fileReader = new FileReader(ymlSchema)) {
			...
			if (yamlSpecifications != null) {
				...
				if (yamlSpecifications.get(YmlUncertaintySchema.CONF_COM) instanceof Map) {
					return true;
				}

				// retrieve parameters
				if (yamlSpecifications.containsKey(YmlUncertaintySchema.CONF_UNCERTAINTY_PARAMETER)) {
					return true;
				}
			}
		} catch (Exception e) {
	...
	private UncertaintySpecification read(File ymlSchema) throws CredibilityException, IOException {
		...
		if (yamlSpecifications != null) {
			...
			if (yamlSpecifications.get(YmlUncertaintySchema.CONF_COM) instanceof Map) {
				yamlUncertaintySpecifications = (Map<?, ?>) yamlSpecifications.get(YmlUncertaintySchema.CONF_COM);
			}

			if (yamlUncertaintySpecifications != null) {

				// Retrieve parameters
				if (yamlUncertaintySpecifications.get(YmlUncertaintySchema.CONF_UNCERTAINTY_PARAMETER) instanceof Map) {
					@SuppressWarnings("unchecked")
					Map<String, Map<String, Object>> yamlParameters = (Map<String, Map<String, Object>>) yamlUncertaintySpecifications
							.get(YmlUncertaintySchema.CONF_UNCERTAINTY_PARAMETER);
					try {
					...
					logger.warn(errorMessage, YmlUncertaintySchema.CONF_UNCERTAINTY_PARAMETER);
				}
			}
		}
