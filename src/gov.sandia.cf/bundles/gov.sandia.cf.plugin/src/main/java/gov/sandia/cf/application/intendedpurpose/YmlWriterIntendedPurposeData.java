 ...
 *
 */
public class YmlWriterIntendedPurposeData implements IYmlDataWriter {

	/**
	 * the logger
	 */
	private static final Logger logger = LoggerFactory.getLogger(YmlWriterIntendedPurposeData.class);
	...
	public void writeData(final Yaml yamlEngine, final File cfDataFile, final Map<ExportOptions, Object> exportOptions,
		...
		if (!exportOptions.containsKey(ExportOptions.INTENDEDPURPOSE_INCLUDE)
				|| !Boolean.TRUE.equals(exportOptions.get(ExportOptions.INTENDEDPURPOSE_INCLUDE))) {
			logger.warn("Intended Purpose is not selected to be included. Nothing to write into file {}", //$NON-NLS-1$
		...
		try (Writer writer = new StringWriter()) {
			...
			yamlEngine.dump(ExportOptionsMapper.getYmlIntendedPurposeData(exportOptions), writer);
