...
class Task_003_PlanningFieldLongVarcharTest extends AbstractTestDao {
	 ...
	 * the logger
	 */
	public static final Logger logger = LoggerFactory.getLogger(Task_003_PlanningFieldLongVarcharTest.class);
	...
	void test_MigrationTask_NotMigrated() {
		...
		// test
		try {
			boolean changed = new Task_003_PlanningFieldLongVarchar().execute(getDaoManager());
			assertTrue(changed);
		...
		PCMMPlanningParam planningParam = TestEntityFactory.getNewPCMMPlanningParam(getDaoManager(), null);
		planningParam.setName(longText);
		try {
			getDaoManager().getRepository(IPCMMPlanningParamRepository.class).update(planningParam);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}

		PCMMPlanningQuestion planningQuestion = TestEntityFactory.getNewPCMMPlanningQuestion(getDaoManager(), null);
		planningQuestion.setName(longText);
		try {
			getDaoManager().getRepository(IPCMMPlanningQuestionRepository.class).update(planningQuestion);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}

		PCMMPlanningSelectValue planningSelectValue = TestEntityFactory.getNewGenericSelectValue(getDaoManager(),
				PCMMPlanningSelectValue.class, planningParam, IPCMMPlanningSelectValueRepository.class);
		planningSelectValue.setName(longText);
		try {
			getDaoManager().getRepository(IPCMMPlanningSelectValueRepository.class).update(planningSelectValue);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}

		UncertaintyParam uncertaintyParam = TestEntityFactory.getNewUncertaintyParam(getDaoManager(), null, null);
		uncertaintyParam.setName(longText);
		try {
			getDaoManager().getRepository(IUncertaintyParamRepository.class).update(uncertaintyParam);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}

		UncertaintySelectValue uncertaintySelectValue = TestEntityFactory.getNewGenericSelectValue(getDaoManager(),
				UncertaintySelectValue.class, uncertaintyParam, IUncertaintySelectValueRepository.class);
		uncertaintySelectValue.setName(longText);
		try {
			getDaoManager().getRepository(IUncertaintySelectValueRepository.class).update(uncertaintySelectValue);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}
	}
