...
/**
 * The Class Task_007_ARGParametersLongVarcharFieldsTest.
 *
 * @author Didier Verstraete
 */
class Task_007_ARGParametersLongVarcharFieldsTest extends AbstractTestDao {

	/**
	 * the logger
	 */
	public static final Logger logger = LoggerFactory.getLogger(Task_007_ARGParametersLongVarcharFieldsTest.class);
	...
	void test_MigrationTask_NotMigrated() {
		...
		// test
		try {
			boolean changed = new Task_007_ARGParametersLongVarcharFields().execute(getDaoManager());
			assertTrue(changed);
		...
		ARGParameters argParam = TestEntityFactory.getNewARGParameters(getDaoManager());
		argParam.setOutput(longText);
		argParam.setParametersFilePath(longText);
		argParam.setStructureFilePath(longText);
		try {
			getDaoManager().getRepository(IARGParametersRepository.class).update(argParam);
