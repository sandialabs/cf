...
class Task_002_RichTextContentTest extends AbstractTestDao {
	 ...
	 * the logger
	 */
	public static final Logger logger = LoggerFactory.getLogger(Task_002_RichTextContentTest.class);
	...
	void test_MigrationTask_NotMigrated() {
		...
		// test
		try {
			boolean changed = new Task_002_RichTextContent().execute(getDaoManager());
			assertTrue(changed);
		...
		assessment.setComment(longText);
		try {
			getDaoManager().getRepository(IPCMMAssessmentRepository.class).update(assessment);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}

		PCMMEvidence evidence = TestEntityFactory.getNewPCMMEvidence(getDaoManager(), null, null, null);
		evidence.setDescription(longText);
		try {
		...
		qoi.setDescription(longText);
		try {
			getDaoManager().getRepository(IQuantityOfInterestRepository.class).update(qoi);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}

		Criterion criterion = TestEntityFactory.getNewCriterion(getDaoManager(), null);
		criterion.setValue(longText);
		try {
			getDaoManager().getRepository(ICriterionRepository.class).update(criterion);
		} catch (CredibilityException e) {
			fail(e.getMessage());
		}
	}
